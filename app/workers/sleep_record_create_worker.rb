class SleepRecordCreateWorker
  ...
  def perform(wake_up_clock_in_id)
    ...
    duration = (wake_up_clock_in.created_at - last_sleep_clock_in.created_at).to_i

    SleepRecord.create(user_id: user_id, wake_up_clock_in: wake_up_clock_in, sleep_clock_in: last_sleep_clock_in, duration: duration)
  end
end
